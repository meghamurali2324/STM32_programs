...
int main(void)
{
    ...
    led1.GPIO_PinConfig.GPIO_PinSpeed = GPIO_SPEED_LOW;
    led1.GPIO_PinConfig.GPIO_PinPuPdControl = GPIO_NO_PUPD;

    led2.pGPIOx = GPIOA;										    //  LED2 : PORTA7
    ...
    while(1)
    {
        ...
        GPIO_WriteToOutputPin(GPIOA, GPIO_PIN_NO_6, ENABLE);						        //LED1 ON
        GPIO_WriteToOutputPin(GPIOA, GPIO_PIN_NO_7, DISABLE);								//LED2 OFF
        delay();

        GPIO_WriteToOutputPin(GPIOA, GPIO_PIN_NO_6, DISABLE);						 		// LED1 OFF
        GPIO_WriteToOutputPin(GPIOA, GPIO_PIN_NO_7, ENABLE);								//LED ON
        delay();
    }
}
